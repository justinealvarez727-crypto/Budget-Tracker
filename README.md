...
Pick any name/region and a database password (save it somewhere safe — you won't need it
day-to-day, only if you ever connect a raw Postgres client).
Once the project is ready, open SQL Editor in the left sidebar → New query.
Paste the entire contents of `supabase/schema.sql` (included in this project) and click Run.
...
Open Project settings → API. Copy two values — you'll need them next:
Project URL
anon public key
...
Push this project to a GitHub repository (create a new empty repo, then):
```bash
   git init
   git add .
...
Click Deploy. You'll get a URL like `https://your-app.vercel.app` — that's your live app.
...
Each update ships as one small SQL migration (run once in Supabase) plus new code
(pushed to GitHub, which Vercel auto-deploys). Run migrations in order — each is
safe to run on a database that already has data; they only ever add columns/tables,
never remove or rewrite your existing rows.
Migrations so far:
`supabase/migration_002_features.sql` — category icons, budget assignment, savings goals
`supabase/migration_003_adjustments.sql` — balance adjustment entries
To apply: Supabase → SQL Editor → new query → paste the file's contents → Run.
Then push the updated code (`git add . && git commit -m "Update" && git push`) —
